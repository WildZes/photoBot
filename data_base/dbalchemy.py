# Статья для отправки изображения в БД
...
class DBManager(metaclass=Singleton):
    ...
    def __init__(self):
        ...
        if self._session.query(Category).first()==None:
    ...
    def _add_orders(self, quantity, product_id, user_id):
        """
        Метод заполнения заказов
        """
        all_id_product = self.select_all_product_id()
        print(all_id_product, quantity, product_id)
    ...
    def select_all_product_id(self):
        """
        Возвращает все айди товара в заказе
        """
    ...
    def select_single_product_quantity(self, rownum):
        ...
        который определяется при выборе товара в интерфейсе
        """
    ...
    def update_order_value(self, product_id, name, value):
        ...
        в соответствии с номером товара - rownum
        """
        self._session.query(Order).filter_bu(product_id=product_id).update({name: value})
        self._session.commit()
        self.close()
    ...
    def update_order_value(self, product_id, name, value):
        """
        Обновляет данное указанной позиции заказа
        в соответствии с номером товара - rownum
        """
    ...
    def get_rand_photo(self):
        rand = random.randrange(0, self._session.query(Image).count())
        row = self._session.query(Image)[rand]
        self.close()
        return row.img

    def get_images_by_category(self, category):
        result = self._session.query(Image).filter_by(category_id=category)
        self.close()
        return result
